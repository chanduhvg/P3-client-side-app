...
QUnit.test('TEST multiplication', assert => {
  assert.equal(multiplyBy(2,2,2,2), 8, 'multiplication value')
  assert.equal(multiplyBy(-2,2,2,2), -8, 'Negative multiplication value')
  assert.equal(multiplyBy(0,8,2,2), 0, 'Zero multiplication value')
  assert.equal(multiplyBy(2,2,2,2.2), 17.6, 'decimal multiplication value')
  assert.equal(multiplyBy(0,0,0,0), 0, 'zero multication value')
})
